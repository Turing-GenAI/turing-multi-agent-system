...
class ComplianceService:
    ...
    async def analyze_compliance(self, review_input: ComplianceReviewInput) -> List[ComplianceIssue]:
        ...
        try:
            ...
            if chunk_issues and len(chunk_issues) > 0:
                logger.info(f"Found {len(chunk_issues)} issues using agentic chunk-based analysis")
        ...
        try:
            ...
            if direct_issues and len(direct_issues) > 0:
                logger.info(f"Found {len(direct_issues)} issues using whole-document analysis")
                ...
                logger.info("Whole-document analysis found no additional issues")
        except Exception as e:
        ...
        logger.info(f"Found {len(deduplicated_issues)} compliance issues after deduplication (from {len(all_issues)} original issues)")
    ...
    def _deduplicate_issues(self, issues: List[ComplianceIssue]) -> List[ComplianceIssue]:
        ...
        for text, group in text_groups.items():
            if len(group) == 1:
                ...
                logger.info(f"Found {len(group)} issues for the same clinical text: '{text[:30]}...'")
                ...
                if high_confidence:
                    ...
                    logger.info(f"Selected high confidence issue out of {len(group)} duplicates")
                else:
                    # No high confidence issues, use the first one
                    selected = group[0]
                    logger.info(f"Selected first issue (all are low confidence) out of {len(group)} duplicates")
                
                deduplicated.append(selected)
        
        logger.info(f"Deduplication reduced {len(issues)} issues to {len(deduplicated)} unique issues")
        return deduplicated
    ...
    def _is_similar_issue(self, issue1: ComplianceIssue, issue2: ComplianceIssue) -> bool:
        ...
        if issue1.clinical_text.strip() == issue2.clinical_text.strip():
            logger.debug(f"Found exact clinical text match - marking as duplicate")
            return True
        ...
        if exact_clinical_overlap:
            logger.debug(f"Found high clinical text overlap ({clinical_overlap:.2f}) - marking as duplicate")
            return True
    ...
    async def _analyze_full_documents(self, clinical_doc_content: str, compliance_doc_content: str) -> List[ComplianceIssue]:
        ...
        human_prompt = get_whole_document_analysis_prompt(clinical_doc_content, compliance_doc_content)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    found_categories = [key for key in data.keys() if key != 'issues' and isinstance(data[key], list)]
                    
                    if found_categories:
                        logger.info(f"Found category-based structure with {len(found_categories)} categories")
                        combined_issues = []
                        
                        # Collect issues from all categories dynamically
                        for category in found_categories:
                            category_issues = data[category]
                            logger.info(f"Found '{category}' with {len(category_issues)} issues")
                            combined_issues.extend(category_issues)
                        
                        llm_issues = [LLMComplianceIssue(**issue) for issue in combined_issues]
                        logger.info(f"Combined {len(llm_issues)} issues from all categories")
                    else:
                ...
                try:
                    ...
                    if ('"compliance_issues":' in json_str or '"findings":' in json_str) and '"issues":' not in json_str:
                        ...
                        if '"compliance_issues":' in json_str:
                            fixed_json_str = json_str.replace('"compliance_issues":', '"issues":') 
                            logger.info(f"Adapting JSON structure: changing 'compliance_issues' key to 'issues'")
                        elif '"findings":' in json_str:
                            fixed_json_str = json_str.replace('"findings":', '"issues":') 
                            logger.info(f"Adapting JSON structure: changing 'findings' key to 'issues'")
                            
                        try:
                            issue_list = ComplianceIssueList.model_validate_json(fixed_json_str)
                            llm_issues = issue_list.issues
                            logger.info(f"Successfully adapted JSON structure with {len(llm_issues)} issues found")
                        except Exception as adapt_err:
                            # If adaptation fails, fall through to original attempt
                            logger.warning(f"Adaptation failed: {adapt_err}. Trying original validation.")
                            issue_list = ComplianceIssueList.model_validate_json(json_str)
                            llm_issues = issue_list.issues
                    else:
                    ...
                    try:
                        data = json.loads(json_str)
                        logger.info(f"Parsed JSON keys: {', '.join(data.keys())}")
                        
                        # Check for all possible keys LLM might use
                        if 'compliance_issues' in data:
                            logger.info(f"Found 'compliance_issues' key with {len(data['compliance_issues'])} issues")
                            llm_issues = [LLMComplianceIssue(**issue) for issue in data['compliance_issues']]
                        elif 'findings' in data:
                            logger.info(f"Found 'findings' key with {len(data['findings'])} issues")
                            llm_issues = [LLMComplianceIssue(**issue) for issue in data['findings']]
                        else:
                            # Fall back to the original approach
                            issues_array = data.get("issues", [])
                            logger.info(f"Found 'issues' key with {len(issues_array)} issues")
                            llm_issues = [LLMComplianceIssue(**issue) for issue in issues_array]
                            
                    except Exception as json_err:
                        logger.error(f"JSON parsing failed: {json_err}. Unable to extract issues.")
                        llm_issues = []
                    ...
                    try:
                        ...
                        try:
                            ...
                            if match:
                                ...
                                logger.debug(f"Set confidence to {confidence_score} based on numerical value {confidence_value}")
                            else:
                                logger.warning(f"No numerical confidence value found in response: '{confidence_text}'")
    ...
    async def _apply_insertion(self, target_text: str, content_to_insert: str, surrounding_context: str) -> str:
        ...
        try:
            ...
            human_prompt = get_insertion_content_human_prompt(target_text, content_to_insert, surrounding_context)
