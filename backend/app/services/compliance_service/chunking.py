...
try:
    ...
    logging.warning("openai package not installed. AgenticChunker will not work.")
...
class TokenChunker(BaseChunker):
    ...
    def split_with_offsets(self, text: str) -> List[TextWithOffset]:
        ...
        for chunk in chunks:
            ...
            if found_pos == current_pos and text.find(chunk, current_pos) == -1:
                logger.warning(f"Using approximate position for token-based chunk: '{chunk[:30]}...'")
...
class SentenceChunker(BaseChunker):
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        try:
            ...
            logger.error(f"NLTK chunking failed: {e}. Falling back to recursive chunking.")
    ...
    def split_with_offsets(self, text: str) -> List[TextWithOffset]:
        ...
        try:
            ...
            for chunk in chunks:
                ...
                if found_pos == current_pos and text.find(chunk, current_pos) == -1:
                    logger.warning(f"Using approximate position for sentence chunk: '{chunk[:30]}...'")
            ...
            logger.error(f"NLTK chunking with offsets failed: {e}. Falling back to recursive chunking.")
            # Fallback to recursive chunking
            fallback = RecursiveChunker(self.chunk_size, self.chunk_overlap)
...
class ChunkContent(BaseModel):
    ...
    rewritten: str = Field(..., description="The rewritten text that combines and improves the grouped sentences")


class ChunkGroupsDirect(BaseModel):
    """Represents the structured output from the LLM chunking."""
    chunks: List[ChunkContent] = Field(..., description="List of rewritten text chunks")
...
class AgenticChunker(BaseChunker):
    ...
    def __init__(self, chunk_size: int = 3500, chunk_overlap: int = 250, batch_size: int = 10):
        ...
        if not OPENAI_AVAILABLE:
            logging.warning("OpenAI package not installed, using fallback chunker instead.")
    ...
    def split_into_sentences(self, text: str) -> List[str]:
        ...
        for paragraph in paragraphs:
            if len(paragraph) > 3000:  # Long paragraph
                ...
                sections = [s.strip() for s in paragraph.split('\n') if s.strip()]
    ...
    def process_with_llm(self, text: str) -> List[str]:
        ...
        for start in range(0, len(chunks), self.batch_size):
            ...
            if result and result.chunks:
                all_results.extend([chunk.rewritten for chunk in result.chunks])
            else:
                # If LLM processing fails, include the original text
                all_results.extend(batch_chunks)
                
        if not all_results:
            logging.warning("No chunks produced by LLM, falling back to basic chunking")
            return self.fallback_chunker.split_text(text)
    ...
    def split_text(self, text: str) -> List[str]:
        ...
        try:
            ...
            logging.error(f"Error in agentic chunking: {e}, falling back to recursive chunking")
    ...
    def split_with_offsets(self, text: str) -> List[TextWithOffset]:
        ...
        try:
            ...
            for chunk in chunks:
                ...
                if found_pos == current_pos and text.find(chunk, current_pos) == -1:
                    logging.warning(f"Using approximate position for LLM-generated chunk: '{chunk[:30]}...'")
            ...
            logging.error(f"Error in agentic chunking with offsets: {e}, falling back to recursive chunking")
