        ...
        if not force_refresh:
            ...
            if cache_key in analysis_results and analysis_results[cache_key].get('issues'):
                ...
                if isinstance(issues, list) and len(issues) > 0:
                    print(f"Returning cached analysis for documents {clinical_doc_id} and {compliance_doc_id} with {len(issues)} issues")
            ...
            for review in reviews_db:
                if (review.get('clinical_doc_id') == clinical_doc_id and 
                    ...
                    if isinstance(issues, list) and len(issues) > 0:
                        print(f"Returning analysis from existing review with {len(issues)} issues")
                        return {
            ...
            print(f"Force refreshing analysis for documents {clinical_doc_id} and {compliance_doc_id}")
        
        # Either no valid cached results, or force_refresh is true - create a new analysis
        print(f"Creating new analysis for documents {clinical_doc_id} and {compliance_doc_id}")
        
        # Get document content from document service
        clinical_doc_content = document_service.get_document_content(clinical_doc_id)
        compliance_doc_content = document_service.get_document_content(compliance_doc_id)
