...
from app.utils.tool_nodes import (
    process_human_feedback_chain, 
    process_human_feedback_for_sub_activities_chain, 
    process_human_feedback_for_sub_activities_chain_2
)
...
def run_graph_first_run(inputs, config_, scratchpad_filename=None, save_image_flag=False):
    """
...
def run_graph_after_interruption(config_, state_before_interruption, scratchpad_filename=None, interruption_inputs={}):
    """
    Executes the trial supervisor graph and processes events in a loop until completion or interruption.
    ...
    if purpose == "get_user_feedback":
        ...
        current_config = graph.get_state(config_, subgraphs=True).tasks[0].state.config
        graph.update_state(
