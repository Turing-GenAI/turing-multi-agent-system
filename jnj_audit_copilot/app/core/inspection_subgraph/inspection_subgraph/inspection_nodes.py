...
import uuid
# from langgraph.graph import clear_interrupt_cache
...
class inspectionNodes:
    ...
    def validate_sub_activity_node(self, state: InspectionAgentState) -> dict:
        ...
        if not feedback_flag:
            extra_state_for_next_step = {
                ...
                "purpose": "get_user_feedback_for_sub_activities",
            }
    ...
    def interrupt_for_planner_feedback_node(self, state: InspectionAgentState) -> dict:
        logger.debug("Calling function : interrupt_for_planner_feedback_node...")
        
        # clear_interrupt_cache() 
        if FEEDBACK_FOR_PLANNER:
            # sub_activities_list = state["final_sub_activities"].sub_activities
            result = interrupt(
                {
                    "task": "Review the sub-activities generated by the LLM and make any necessary changes.",
                    "final_sub_activities": state["final_sub_activities"],
                    "interrupt_id": str(uuid.uuid4()),
                }
            )
            return Command(goto="selfrag_subgraph", resume={
                "final_sub_activities": result["final_sub_activities"],
            })
        else:
            feedback = "y"
            return {
    ...
    # def add_human_in_the_loop_for_validating_findings_node(self, state: InspectionAgentState):
    #     logger.debug("Calling function : add_human_in_the_loop_for_validating_findings_node ...")
    #     if "human_feedback" in state.keys():
    #         is_approved = interrupt(
    #             {
    #                 "question": "Do you approve of the above findings? Type 'y' to continue; otherwise, explain.",
    ...
    #         )
    #         if is_approved:
    #             return Command(goto = "discrepancy_data_generator")
    #         else:
    #             return Command(goto = "generate_findings_agent")
    #     else:
    #         return Command(goto = "discrepancy_data_generator")
